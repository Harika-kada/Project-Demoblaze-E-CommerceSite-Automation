...
public class CheckoutTests {
	...
	private Map<String, String> getScenario(String userType, String address, String payment) {
		...
		return dataList.stream()
				.filter(row -> row.get("User Type").equalsIgnoreCase(userType)
						&& row.get("Shipping Address").equals(address) && row.get("Payment Method").equals(payment))
				.findFirst().orElseThrow(() -> new RuntimeException(
						"Scenario not found for: " + userType + ", " + address + ", " + payment));
	}
	...
	private void runCheckoutTest(Map<String, String> data) {
		...
		Assert.assertEquals(actualMessage.trim(), expectedMessage.trim(),
				"Validation failed for user type: " + userType);
	}
